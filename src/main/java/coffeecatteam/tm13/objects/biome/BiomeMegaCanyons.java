...
public class BiomeMegaCanyons extends BiomeBase {
    ...
    public static final SurfaceBuilderConfig SURFACE = new SurfaceBuilderConfig(SAND, Blocks.SANDSTONE.getDefaultState(), GRAVEL);

    public BiomeMegaCanyons(String id) {
        super(id, new BiomeBuilder().surfaceBuilder(new CompositeSurfaceBuilder<>(DEFAULT_SURFACE_BUILDER, SURFACE)).precipitation(RainType.NONE).category(Category.EXTREME_HILLS).depth(0.2F).scale(0.5F).temperature(1.0F).downfall(1.0F).waterColor(14859588).waterFogColor(3353359).parent(null));
        ...
        this.addFeature(GenerationStage.Decoration.UNDERGROUND_STRUCTURES, createCompositeFeature(Feature.LAKES, new LakesConfig(Blocks.WATER), LAKE_WATER, new LakeChanceConfig(2)));
        ...
        this.addFeature(GenerationStage.Decoration.UNDERGROUND_STRUCTURES, createCompositeFeature(Feature.DUNGEONS, IFeatureConfig.NO_FEATURE_CONFIG, DUNGEON_ROOM, new DungeonRoomConfig(20)));
